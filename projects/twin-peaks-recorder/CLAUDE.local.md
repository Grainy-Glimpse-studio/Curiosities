...
## 📤 Upload Tape - 转换工具卡带（2026-03-09）

### 概述

第二个 default 磁带，用于文件转换功能。点击打开带 Tabs 的浮动窗口。

### Tab 结构

```
┌─────────────────────────────────────────────────────────────┐
│  [ Audio/Video → Text ]  [ SRT → TTS ]                      │
├─────────────────────────────────────────────────────────────┤
│                        Tab 内容                              │
└─────────────────────────────────────────────────────────────┘
```
...
### Tab 1: Audio/Video → Text

**功能**：上传音频/视频文件，转成文字

**支持格式**：MP3, MP4, WAV, M4A, WebM 等

**转写 API**：Deepgram（后台部署，用户无需填 key）
...
│               └─ Word (.docx)                               │
│                                                             │
│  [保存为新磁带]  ← 创建新的 memo，带 transcription           │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**技术实现**：
- 视频转音频：ffmpeg.wasm（纯前端）
- 音频转文字：Deepgram API（后台代理）
- 支持导入 BWF 文件，读取 TimeReference 作为时间偏移

---

### Tab 2: SRT → TTS

**功能**：上传 SRT 字幕文件，用 TTS 生成配音音频
...
**左侧设置区**：
- API 选择：Radio buttons
- Voice 选择：Radio buttons（显示用户自定义的声音名字）
- [管理声音] 按钮：跳转到 Account Settings
...
**Voice 列表逻辑**：
- 只显示当前选中 API 下的声音
- 切换 API 时，Voice 列表自动更新
- 如果用户还没配置任何声音，显示提示

---

### Account Settings - TTS 配置

**数据结构**：
```typescript
// 用户保存的 TTS 配置（存在 Supabase）
interface UserTTSConfig {
  id: string;
  user_id: string;
  api_provider: 'elevenlabs' | 'fish_audio';
  api_key: string;  // 加密存储
  models: TTSModel[];
}
...
interface TTSModel {
  id: string;
  model_id: string;      // API 真实的 model ID
  label: string;         // 用户自定义的名字，如 "我的中文女声"
  language?: string;
}
```
...
| 格式 | 说明 | 用途 |
|------|------|------|
| MP3/WAV (带静音) | 包含完整时间线，静音间隔保留 | 直接拖入剪辑软件 |
| BWF (TimeReference) | 紧凑音频 + 起始时间码 | 剪辑软件自动定位 |

**BWF TimeReference 计算**：
- 采样率 44.1kHz
- 第一句从 5 秒开始 → TimeReference = 5 × 44100 = 220,500
...
1. ✅ 记录设计文档
2. 🔄 Tab 1: Audio/Video → Text
3. ⏳ Account Settings TTS 管理
4. ⏳ Tab 2: SRT → TTS

---

## 🚧 FloatingTranscript 三面展开布局（2026-03-09 规划中）
...
**工具栏按钮改动**：
- 按钮名称：从 "AI" 改为 "Shortcut"
- 按钮颜色：从紫色改为普通白色
- 选中状态：有视觉效果区分

**从工具栏移入的功能**：
- Distill（蒸馏/提炼）→ 移到右侧面板
- Clear（清理）→ 移到右侧面板
...
**AI Shortcuts 特点**：
1. 每个 shortcut 对应一个调试好的 prompt
2. 处理后显示：结果 + 保留原文对比
3. 用户确认满意后再删掉原文

**Cleanup 说明**：
- 昨天做的「去口癖词」是正则匹配（无需 AI）→ 这个叫 "Clear"
- 用户需要的 **AI Cleanup**：整理思路、去掉重复意思、让表达更清晰
- 这个需要真正调用 AI API
...
**名称**：TOC（Table of Contents）

**功能**：
- 显示 Markdown 文档的标题结构
- 点击标题可跳转到对应位置
- 类似 Notion / Obsidian 的侧边目录

**已知 Bug**：
- ⚠️ TOC 不是实时更新的 - 编辑文本后目录不会自动刷新
- 有时需要关闭窗口再打开才能看到变化
- 需要修复：监听编辑器内容变化，实时更新 TOC
...
所有四个区域（主内容 + 左侧 + 右侧 + 下方）必须使用 **完全相同** 的玻璃质感样式：
...
| 按钮 | 行为 |
|------|------|
| **Export** | 展开整个下方面板（两列布局） |
| **Join** | 菜单**向下**展开（在窗口外部，不遮挡内容） |
| **Save** | 无菜单，直接保存 |

**互斥逻辑**：
- Export 面板和 Join 下拉菜单不能同时打开
- 打开一个会自动收起另一个
- 保持界面整洁

**当前问题**：
- Join 菜单向上展开会被内容遮挡，看不清
- 改为向下展开（在浮动窗口外部）可解决
...
**当前问题**：
- 录音时自动生成的 tags 没有实际用处
- 用户无法手动管理标签
- Archive 的 Tag 筛选器只是 UI，功能不完整
...
当前录音机只有**录音模式**，没有**播放模式**。

用户希望：
- 从 Archive 选一个磁带 → 加载到主页录音机 → 在主页播放
- 播放时可以用录音机按钮控制（Play/Pause/Stop/Forward/Rewind）
- 播放时显示进度、Flow 效果

### 核心问题

**同一套按钮，两种模式：**
1. 录音模式：Record, Pause, Stop, Slate(打板)
2. 播放模式：Play, Pause, Stop, Forward, Rewind

### 待设计

- [ ] 按钮布局：Play/Pause 合并？Slate 放哪？
- [ ] 状态机：录音/播放/闲置 三种状态的切换
- [ ] UI 反馈：当前是录音还是播放模式
- [ ] 从 Archive 加载磁带到主页的交互
