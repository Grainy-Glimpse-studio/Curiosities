...
const TapeDrawer: React.FC<TapeDrawerProps> = ({
  ...
  // Easter egg: ARCHIVE <-> TWINPEAKS transition
  const [bgWord, setBgWord] = useState<'ARCHIVE' | 'TWINPEAKS'>('ARCHIVE');
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-black overflow-hidden">
        ...
        {/* Background Text - ARCHIVE / TWINPEAKS (stacked vertically, cross-dissolve on hover) */}
        <div className="absolute right-12 md:right-16 lg:right-24 top-1/2 -translate-y-1/2 select-none flex flex-col items-center z-20 pointer-events-auto">
          {/* Both words rendered, cross-dissolve via opacity */}
          {(['ARCHIVE', 'TWINPEAKS'] as const).map((word) => {
            ...
            return (
              <div
                ...
                style={{
                  position: word === 'TWINPEAKS' ? 'absolute' : 'relative',
                  opacity: wordOpacity,
              ...
              >
                {word.split('').map((letter, i) => {
                  ...
                  let glowIntensity = 0;
                  const refIndex = word === 'ARCHIVE' ? i : i + 10; // Offset for TWINPEAKS refs
                  ...
                  return (
                    <span
                      ...
                      style={{
                        color: `rgba(255,255,255,${0.03 + glowIntensity * 0.15 + hoverBoost})`,
                        textShadow: (glowIntensity > 0 || isHovered)
                          ? `0 0 ${30 + glowIntensity * 40}px rgba(255,255,255,${glowIntensity * 0.5 + hoverBoost}), 0 0 ${60 + glowIntensity * 80}px rgba(255,255,255,${glowIntensity * 0.3 + hoverBoost * 0.5})`
                          : 'none',
                      }}
                      onMouseEnter={() => {
                        if (isCurrentWord && wordOpacity > 0.5) {
                          setHoveredLetters(prev => {
                            const next = new Set(prev);
                            next.add(i);
                            // Check if all letters have been hovered
                            if (next.size >= word.length) {
                              // Trigger cross-dissolve transition
                              setTimeout(() => {
                                setWordTransition(1);
                                setTimeout(() => {
                                  setBgWord(bgWord === 'ARCHIVE' ? 'TWINPEAKS' : 'ARCHIVE');
                                  setWordTransition(0);
      ...
      <div className="absolute inset-0 z-20">
        ...
        <div className="absolute inset-0 overflow-y-auto pt-20 px-8 pb-8 scrollbar-hide">
