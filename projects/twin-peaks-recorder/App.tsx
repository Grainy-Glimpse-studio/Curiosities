...
const App: React.FC = () => {
  ...
  const [trailRender, setTrailRender] = useState(0);
  ...
  const [speechLang, setSpeechLang] = useState<'auto' | 'zh' | 'en'>('auto'); // 语音识别语言
  const [openTranscripts, setOpenTranscripts] = useState<Memo[]>([]); // 多个浮动窗口
  const [showAboutPage, setShowAboutPage] = useState(false); // About 页面
  const [aboutPageSource, setAboutPageSource] = useState<'homepage' | 'archive' | 'transcript'>('homepage'); // About 页面来源
  const [starVisible, setStarVisible] = useState(false); // 流星是否可见
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white overflow-hidden relative font-sans select-none cursor-none">
      ...
      {/* Shooting Star - 点击可进入 About 页面 */}
      <ShootingStar
        onCatch={() => {
          setAboutPageSource('homepage');
          setShowAboutPage(true);
        }}
        disabled={isDrawerOpen || showAboutPage || recorderState === RecorderState.RECORDING}
      ...
      {/* Floating Transcript Windows - 支持多个窗口 */}
      {openTranscripts.map((memo, index) => (
        <FloatingTranscript
          ...
          initialOffset={index * 30} // 每个窗口稍微错开位置
          onOpenAbout={() => {
            setAboutPageSource('transcript');
            setShowAboutPage(true);
          }}
          creatorEmail="diane@twinpeaks.fm" // 你的邮箱，之后可以改
        />
      ))}

      {/* Toggle Labels - Top Left */}
      {recorderState === RecorderState.RECORDING && (
        <div className="fixed top-6 left-6 z-50 flex items-center gap-6">
          {/* Toggle Floating Words */}
          <span
      ...
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden bg-black">
        ...
        {React.useMemo(() => {
          ...
          for (let i = 0; i < 80; i++) {
            ...
            const duration = 2 + pseudoRandom(i * 7) * 5; // 更快：2-7秒
            const delay = pseudoRandom(i * 11) * -8;
            const isBright = pseudoRandom(i * 17) > 0.7; // 30% 的星星更亮
      ...
      <div className={`relative z-10 h-screen flex flex-col transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] ${isDrawerOpen ? 'scale-95 opacity-40 blur-md grayscale pointer-events-none' : 'scale-100 opacity-100'}`}>
        ...
        {/* 提示文字 - 流星出现时或录音时 */}
        <AnimatePresence>
          {starVisible && recorderState !== RecorderState.RECORDING && (
            <motion.div
              initial={{ opacity: 0 }}
            ...
            >
              <p className="text-[10px] text-white/40 font-mono tracking-[0.3em] uppercase">
      ...
      <TapeDrawer
        ...
        onOpenAbout={() => {
          setAboutPageSource('archive');
          setIsDrawerOpen(false);
          setTimeout(() => setShowAboutPage(true), 300); // 等抽屉关闭动画结束
        }}
      />

      {/* Custom Glowing Cursor - Soft ethereal glow */}
      <div className="fixed inset-0 pointer-events-none z-[9999]">
        {/* Trail glow orbs - only show on main page, not in Archive */}
