...
export function Particles({ mousePosRef }: { mousePosRef: React.MutableRefObject<THREE.Vector3 | null> }) {
  ...
  const clearExplosions = useGameStore((state) => state.clearExplosions);
  ...
  const spawnParticle = (pos: THREE.Vector3, colorHex: string, isExplosion = false) => {
    ...
    if (isExplosion) {
      ...
      const r = 5.0 + Math.random() * 15.0; // Explosion speed
    ...
    if (explosions.length > 0) {
      explosions.forEach(exp => {
        ...
        for (let i = 0; i < 1000; i++) {
          spawnParticle(pos, exp.color, true);
        }
      });
      clearExplosions();
    }
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < MAX_PARTICLES; i++) {
      ...
      p.velocity.y -= delta * 2.0; // Gravity
